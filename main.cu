...
__global__ void KernelGPU2(const float* array, float* arrayp1, const int Nx, const int Ny, const float dx2, const float dy2, const float alpha, const float dt){
  ...
  // Borde derecho
  if (s_i != block_size_x + 1  && s_j == block_size_y && j != Ny-1){
...
//--------------- variables de CPU -------------------
  ...
  // Varibles para sacar tiempo promedio
  float sum = 0;
  float iter = 0;
...
//----------------------- CPU ------------------------
  ...
  for (int n = 0; n <= t; n++){
    ...
    // Printea el arreglo cada 20 iteraciones.
      ...
      char filename[64];
      sprintf(filename, "archivos/CPU/transmition_CPU_%03d.txt", i);
...
//---------------- variables de GPU -------------------
  ...
  for (int n = 0; n <= t; n++){
    ...
    // Printea el arreglo cada 20 iteraciones.
      ...
      int i = n/outputEveryTime;
      char filename[64];
      sprintf(filename, "archivos/GPU/K1/transmition_GPU_%03d.txt", i);
  ...
  t_prom = sum/iter;
  printf("Tiempo para GPU Kernel 1: %f [ms]\n", t_prom);
...
//---------------- variables de GPU -------------------
  ...
  for (int n = 0; n <= t; n++){
    ...
    // Printea el arreglo cada 20 iteraciones.
      ...
      int i = n/outputEveryTime;
      char filename[64];
      sprintf(filename, "archivos/GPU/K2/transmition_GPU_%03d.txt", i);
      Write(AhostGPU, Nx, Ny, filename);
		}
  ...
  t_prom = sum/iter;
  printf("Tiempo para GPU Kernel 2: %f [ms]\n", t_prom);
